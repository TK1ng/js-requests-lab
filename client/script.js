...
sayHelloButton.addEventListener('mouseover', changeBtnStyle);
...
sayHelloButton.addEventListener('mouseout', clearBtnStyle)
...
function query() {
    ...
    axios.get(`${baseUrl}/query-test/?name=Taryn&age=33`)
        .then(res => {
...
/*
    ...
    - recieved message that no queries were sent.

    2: Send more than 1 query on the URL -- what happened?
    - returned object with multiple queries
*/
...
function createFood(event) {
    ...
    axios.post(`${baseUrl}/food`, body)
        .then(res => {
            let foodDisplay = document.createElement('p');
            foodDisplay.textContent = res.data[res.data.length - 1]
            document.querySelector('body').appendChild(foodDisplay);
        })
        .catch(err => console.log(err))
...
document.getElementById('submit-btn').addEventListener('click', createFood);

function dislayFood(food) {
    let foodDisplay = document.createElement('p');
    foodDisplay.textContent = res.data
    document.querySelector('body').appendChild(foodDisplay);
}